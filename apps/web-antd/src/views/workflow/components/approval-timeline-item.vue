...
import { getOssByIds } from '#/api/system/oss';
import { renderDict } from '#/utils/render';

import { WF_TASK_STATUS } from './dict';
...
onMounted(async () => {
  ...
  const resp = await getOssByIds(props.item.ext);
  attachmentInfo.value = resp.map((item: any) => ({
    ossId: item.ossId,
    url: item.url,
  ...
  <TimelineItem>
    ...
    <div class="mb-5 ml-2 flex flex-col gap-1">
      <div class="flex items-center gap-1">
        ...
        <component :is="renderDict(item.flowStatus, WF_TASK_STATUS)" />
      </div>
