...
function handleDelegation(userList: SystemUserApi.SystemUser[]) {
  if (userList.length === 0 || !props.task) return;
...
function handleTransfer(userList: SystemUserApi.SystemUser[]) {
  if (userList.length === 0 || !props.task) return;
  const current = userList[0];
  if (!current) return;
  approveWithReasonModal({
...
function handleAddSignature(userList: SystemUserApi.SystemUser[]) {
  if (userList.length === 0 || !props.task) return;
...
function handleReductionSignature(userList: SystemUserApi.SystemUser[]) {
  if (userList.length === 0 || !props.task) return;
  const userIds = userList.map((user) => user.userId);
  Modal.confirm({
    title: '提示',
...
function handleUpdateAssignee(userList: SystemUserApi.SystemUser[]) {
  if (userList.length === 0 || !props.task) return;
  const current = userList[0];
  if (!current) return;
  Modal.confirm({
