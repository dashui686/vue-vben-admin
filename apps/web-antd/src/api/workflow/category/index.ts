...
export function categoryAdd(data: CategoryForm) {
  return requestClient.postWithMsg('/workflow/category', data);
}
...
export function categoryUpdate(data: CategoryForm) {
  return requestClient.putWithMsg('/workflow/category', data);
}
...
export function categoryRemove(id: ID | IDS) {
  return requestClient.deleteWithMsg(`/workflow/category/${id}`);
}
